// Blob store for The Mudda Radio. Reuses the SAME storage account + `feed`
// container as the rest of the family, but keeps every radio asset under a
// per-station prefix (default `radio-mu/`) so the stations never collide.
//
//   radio-mu/program.json        -> the current on-air playlist manifest
//   radio-mu/audio/<hash>.mp3     -> one synthesised segment (hash-cached)
//
// Env:
//   BLOB_CONN    Azure Storage connection string
//   BLOB_PREFIX  blob namespace for this station; default radio-mu/

const BLOB_CONN = process.env.BLOB_CONN;
const CONTAINER = 'feed';

const PREFIX = (process.env.BLOB_PREFIX || 'radio-mu/').replace(/\/*$/, '/');
