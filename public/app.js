/* The Mudda Radio — continuous Hindi-RJ tech & AI stream.
...
(function () {
  ...
  var CAT = {
    concern:  { hi: 'चिंता',  cls: 'bad'     },
    outrage:  { hi: 'आक्रोश', cls: 'ugly'    },
    hope:     { hi: 'उम्मीद', cls: 'good'    },
    pride:    { hi: 'गर्व',   cls: 'good'    },
    reform:   { hi: 'सुधार',  cls: 'good'    },
    idea:     { hi: 'विचार',  cls: 'good'    },
    question: { hi: 'सवाल',   cls: 'bad'     },
  ...
  function renderNowPlaying(seg) {
    ...
    tag.textContent = seg.kind === 'story' ? cat.hi : (state.program ? state.program.station : 'The Mudda Radio');
    ...
    if (seg.kind === 'story') {
      ...
      $('npHi').textContent = (state.program && state.program.tagline) || 'The Mudda Radio';
      $('npEn').textContent = '';
    }
    ...
    document.title = (seg.kind === 'story' && (seg.titleHi || seg.title) ? (seg.titleHi || seg.title) + ' · ' : '') + 'The Mudda Radio';
  }
  ...
  function renderQueue() {
    ...
    for (var k = 0; k < show; k++) {
      ...
      var label = seg.kind === 'story' ? (seg.titleHi || seg.title || 'मुद्दा')
        : (seg.kind === 'ident' ? 'स्टेशन आईडी' : seg.kind === 'signoff' ? 'समापन' : 'बीच का ब्रेक');
      var sub = seg.kind === 'story' ? (seg.source || (cat.hi)) : 'The Mudda Radio';
      row.innerHTML =
